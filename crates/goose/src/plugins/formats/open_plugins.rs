...
//!
//! Per the spec, a single plugin can include any combination of components:
//! skills, hooks, agents, commands, etc. We recognize a checkout as an Open
//! Plugin if either of these is true:
//!
//! 1. It contains a manifest at one of:
//!    - `.goose-plugin/plugin.json` (goose-specific override)
//!    - `.plugin/plugin.json` (the spec's standard location)
//!    - `plugin.json` (root manifest, also accepted for ergonomics)
//! 2. It contains at least one recognized component directory (e.g. `skills/`,
//!    `hooks/hooks.json`, `commands/`, `agents/`).
//!
//! We probe Open Plugins before Gemini so a plugin shipping both formats is
//! treated as Open Plugins.
...
/// Component markers that, on their own, are enough to identify a directory
/// as an Open Plugin even without a manifest.
///
/// `skills/` is intentionally excluded: Gemini extensions also ship a
/// `skills/` directory, so a bare `skills/` checkout is ambiguous and we
/// fall through to the Gemini adapter instead.
...
mod tests {
    ...
    fn bare_skills_directory_is_not_claimed_as_open_plugin() {
        // A `skills/` directory alone is ambiguous (Gemini also uses it).
        // Without an Open Plugins manifest we must not claim it.
        let install_root = tempfile::tempdir().unwrap();
        let repo = tempfile::tempdir().unwrap();
        let skill_dir = repo.path().join("skills").join("audit");
        fs::create_dir_all(&skill_dir).unwrap();
    ...
    fn rejects_repo_with_no_manifest_or_components() {
        let install_root = tempfile::tempdir().unwrap();
        let repo = tempfile::tempdir().unwrap();
        // Just some random file, no recognized component or manifest.
